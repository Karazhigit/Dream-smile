...
export function AppointmentEditorModal({appointment,onClose,onSaved}:{appointment?:Appointment|null;onClose:()=>void;onSaved:(message:string)=>void}){
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-primary-dark/35 p-3 backdrop-blur-[2px] sm:p-6" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
    <div role="dialog" aria-modal="true" aria-labelledby="appointment-dialog-title" className="motion-page-enter my-auto w-full max-w-2xl rounded-[24px] border border-line bg-white p-5 shadow-[0_24px_70px_rgba(15,78,85,.22)] sm:p-7">
      <div className="flex items-start justify-between gap-4"><div className="min-w-0"><h2 id="appointment-dialog-title" className="break-words font-serif text-3xl">{rescheduling?"Перенос записи":"Новая запись"}</h2>{appointment&&<p className="mt-2 break-words text-sm text-muted">{appointment.patientName} · {appointment.appointmentDate} · {appointment.appointmentTime}</p>}</div><button type="button" aria-label="Закрыть" onClick={onClose} className="focus-ring grid h-11 w-11 shrink-0 place-items-center rounded-full border border-line text-xl text-muted hover:border-primary hover:text-primary">×</button></div>
