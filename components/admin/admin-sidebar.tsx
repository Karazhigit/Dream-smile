...
export function AdminSidebar({active,onSelect}:{active:AdminNavId;onSelect?:(section:AdminSection)=>void}){
  ...
  async function logout(){if(signingOut)return;setSigningOut(true);const supabase=createSupabaseAuthBrowserClient();if(supabase)await supabase.auth.signOut();router.replace("/admin/login");router.refresh()}
