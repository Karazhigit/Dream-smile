...
export function AdminSidebar({active,onSelect}:{active:AdminNavId;onSelect?:(section:AdminSection)=>void}){
  const router=useRouter();const[signingOut,setSigningOut]=useState(false);const itemClass=(id:AdminNavId)=>`motion-interactive min-h-11 shrink-0 rounded-xl px-4 text-left text-sm font-bold lg:w-full ${active===id?"bg-white text-primary-dark":"text-white/65 hover:bg-white/10"}`;
