...
export function DoctorSidebar({active}:{active:"appointments"|"schedule"}){
  const router=useRouter();const[signingOut,setSigningOut]=useState(false);const itemClass=(id:string)=>`inline-flex min-h-11 shrink-0 items-center rounded-xl px-4 text-sm font-bold lg:w-full ${active===id?"bg-white text-primary-dark":"text-white/70 hover:bg-white/10"}`;
  async function logout(){if(signingOut)return;setSigningOut(true);const supabase=createSupabaseAuthBrowserClient();if(supabase)await supabase.auth.signOut();router.replace("/doctor/login");router.refresh()}
