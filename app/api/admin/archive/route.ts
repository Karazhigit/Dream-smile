...
export async function GET(request:Request){
  ...
  const from=(page-1)*limit;const{data,error,count}=await query.range(from,from+limit-1);if(error){console.error("[api/admin/archive GET] Supabase query failed",error);return Response.json({error:"Не удалось загрузить архив."},{status:500})}
  ...
  return Response.json({appointments,page,limit,total,totalPages:Math.max(1,Math.ceil(total/limit))});
}
