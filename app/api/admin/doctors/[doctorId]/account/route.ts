...
export async function POST(request:Request,{params}:{params:Promise<{doctorId:string}>}){
  ...
  let body:{email?:string;password?:string};try{body=await request.json()}catch{return Response.json({error:"Проверьте данные аккаунта."},{status:400})}const email=body.email?.trim().toLowerCase();if(!email||email.length>254||!emailPattern.test(email)||!body.password||body.password.length<8||body.password.length>128)return Response.json({error:"Проверьте email и пароль. Пароль должен содержать от 8 до 128 символов."},{status:400});
  ...
  if(createError||!createData.user){const duplicate=createError?.code==="email_exists"||/already|exist|registered/i.test(createError?.message??"");if(!duplicate)safeServerError("api/admin/doctors/account create auth user",createError);return Response.json({error:duplicate?"Не удалось создать аккаунт с этими данными.":"Не удалось создать аккаунт врача."},{status:duplicate?409:500})}
  ...
  if(linkError){const{error:rollbackError}=await admin.auth.admin.deleteUser(user.id);if(rollbackError)safeServerError("api/admin/doctors/account rollback",rollbackError);safeServerError("api/admin/doctors/account link",linkError);return Response.json({error:linkError.code==="23505"?"У этого врача уже есть аккаунт.":"Не удалось связать аккаунт с врачом."},{status:linkError.code==="23505"?409:500})}
...
export async function DELETE(request:Request,{params}:{params:Promise<{doctorId:string}>}){
  ...
  const{data:account,error:accountError}=await admin.from("doctor_accounts").select("user_id").eq("doctor_id",doctorId).maybeSingle();if(accountError){safeServerError("api/admin/doctors/account DELETE lookup",accountError);return Response.json({error:"Не удалось проверить аккаунт врача."},{status:500})}if(!account)return Response.json({error:"Аккаунт врача не подключён."},{status:404});
  const{error:deleteError}=await admin.auth.admin.deleteUser(account.user_id);if(deleteError){safeServerError("api/admin/doctors/account DELETE auth user",deleteError);return Response.json({error:"Не удалось отключить аккаунт врача."},{status:500})}
  const{error:cleanupError}=await admin.from("doctor_accounts").delete().eq("doctor_id",doctorId);if(cleanupError)safeServerError("api/admin/doctors/account DELETE cleanup",cleanupError);
