...
export async function GET(request:Request){
  ...
  if(from&&to&&/^\d{4}-\d{2}-\d{2}$/.test(from)&&/^\d{4}-\d{2}-\d{2}$/.test(to)){
    ...
    return Response.json({availableDates:Object.entries(range).filter(([,slots])=>slots.some(slot=>slot.available)).map(([value])=>value),slotsByDate:range},{headers:noStoreHeaders});
  }
