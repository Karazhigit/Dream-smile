...
export async function GET(request:Request){
  ...
  const{data,error}=await query;if(error){safeServerError("api/doctor/appointments GET",error);return Response.json({error:"Не удалось загрузить записи."},{status:500})}
...
export async function PATCH(request:Request){
  ...
  const auth=await requireDoctorApi();if("response" in auth)return auth.response;let body:{id?:string;status?:string};try{body=await request.json()}catch{return Response.json({error:"Проверьте данные записи."},{status:400})}if(!isUuid(body.id)||body.status!=="completed")return Response.json({error:"Разрешено только завершение подтверждённой записи."},{status:400});
  ...
  if(error){safeServerError("api/doctor/appointments PATCH",error);return Response.json({error:"Не удалось завершить запись."},{status:500})}
